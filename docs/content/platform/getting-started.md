# Getting Started with AutoAGI: Self-Hosting Guide

This tutorial will walk you through the process of setting up AutoAGI locally on your machine.
...
We also offer this in video format. You can check it out [here](https://github.com/KhulnaSoft/AutoAGI?tab=readme-ov-file#how-to-setup-for-self-hosting).
...
The first step is cloning the AutoAGI repository to your computer.
To do this, open a terminal window in a folder on your computer and run:
```
git clone https://github.com/KhulnaSoft/AutoAGI.git
```
...
* Within the repository, clone the submodules and navigate to the `autoagi` directory:
  ```bash
...
* Copy the `.env.example` file available in the `supabase/docker` directory to `.env` in `autoagi`:
  ```
...
* Navigate to `frontend` folder within the `autoagi` directory:
  ```
   cd frontend
  ```
...
You may want to change your encryption key in the `.env` file in the `autoagi/backend` directory.
...
Or run the following command in the `autoagi/backend` directory:

```bash
poetry run cli gen-encrypt-key
```

Then, replace the existing key in the `autoagi/backend/.env` file with the new one.
