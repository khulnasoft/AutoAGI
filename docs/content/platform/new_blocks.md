# Contributing to AutoAGI Agent Server: Creating and Testing Blocks

This guide will walk you through the process of creating and testing a new block for the AutoAGI Agent Server, using the WikipediaSummaryBlock as an example.
...
1. **Create a new Python file** for your block in the `autoagi/backend/backend/blocks` directory. Name it descriptively and use snake_case. For example: `get_wikipedia_summary.py`.
...
The `discriminator` parameter tells AutoAGI which field to look at in the input to determine which type it is.
...
The `APIKeyCredentials` and `OAuth2Credentials` models are defined [here](https://github.com/KhulnaSoft/AutoAGI/blob/master/autoagi/autoagi_libs/autoagi_libs/supabase_integration_credentials_store/types.py).
...
--8<-- "autoagi/backend/backend/integrations/providers.py:ProviderName"
```
</details>
...
--8<-- "autoagi/backend/backend/integrations/oauth/base.py:BaseOAuthHandler1"
--8<-- "autoagi/backend/backend/integrations/oauth/base.py:BaseOAuthHandler2"
--8<-- "autoagi/backend/backend/integrations/oauth/base.py:BaseOAuthHandler3"
--8<-- "autoagi/backend/backend/integrations/oauth/base.py:BaseOAuthHandler4"
--8<-- "autoagi/backend/backend/integrations/oauth/base.py:BaseOAuthHandler5"
--8<-- "autoagi/backend/backend/integrations/oauth/base.py:BaseOAuthHandler6"
```
...
- Adding the handler class to `HANDLERS_BY_NAME` under [`integrations/oauth/__init__.py`](https://github.com/KhulnaSoft/AutoAGI/blob/master/autoagi/backend/backend/integrations/oauth/__init__.py)

```python title="backend/integrations/oauth/__init__.py"
--8<-- "autoagi/backend/backend/integrations/oauth/__init__.py:HANDLERS_BY_NAMEExample"
```

- Adding `{provider}_client_id` and `{provider}_client_secret` to the application's `Secrets` under [`util/settings.py`](https://github.com/KhulnaSoft/AutoAGI/blob/master/autoagi/backend/backend/util/settings.py)

```python title="backend/util/settings.py"
--8<-- "autoagi/backend/backend/util/settings.py:OAuthServerCredentialsExample"
```

[OAuth2 handlers]: https://github.com/KhulnaSoft/AutoAGI/tree/master/autoagi/backend/backend/integrations/oauth
[`BaseOAuthHandler`]: https://github.com/KhulnaSoft/AutoAGI/blob/master/autoagi/backend/backend/integrations/oauth/base.py

#### Adding to the frontend

You will need to add the provider (api or oauth) to the `CredentialsInput` component in [`frontend/src/components/integrations/credentials-input.tsx`](https://github.com/KhulnaSoft/AutoAGI/blob/master/autoagi/frontend/src/components/integrations/credentials-input.tsx).

```ts title="frontend/src/components/integrations/credentials-input.tsx"
--8<-- "autoagi/frontend/src/components/integrations/credentials-input.tsx:ProviderIconsEmbed"
```

You will also need to add the provider to the `CredentialsProvider` component in [`frontend/src/components/integrations/credentials-provider.tsx`](https://github.com/KhulnaSoft/AutoAGI/blob/master/autoagi/frontend/src/components/integrations/credentials-provider.tsx).

```ts title="frontend/src/components/integrations/credentials-provider.tsx"
--8<-- "autoagi/frontend/src/components/integrations/credentials-provider.tsx:CredentialsProviderNames"
```

Finally you will need to add the provider to the `CredentialsType` enum in [`frontend/src/lib/autoagi-server-api/types.ts`](https://github.com/KhulnaSoft/AutoAGI/blob/master/autoagi/frontend/src/lib/autoagi-server-api/types.ts).

```ts title="frontend/src/lib/autoagi-server-api/types.ts"
--8<-- "autoagi/frontend/src/lib/autoagi-server-api/types.ts:BlockIOCredentialsSubSchema"
```

#### Example: GitHub integration

- GitHub blocks with API key + OAuth2 support: [`blocks/github`](https://github.com/KhulnaSoft/AutoAGI/tree/master/autoagi/backend/backend/blocks/github/)

```python title="backend/blocks/github/issues.py"
--8<-- "autoagi/backend/backend/blocks/github/issues.py:GithubCommentBlockExample"
```

- GitHub OAuth2 handler: [`integrations/oauth/github.py`](https://github.com/KhulnaSoft/AutoAGI/blob/master/autoagi/backend/backend/integrations/oauth/github.py)

```python title="backend/integrations/oauth/github.py"
--8<-- "autoagi/backend/backend/integrations/oauth/github.py:GithubOAuthHandlerExample"
```

#### Example: Google integration

- Google OAuth2 handler: [`integrations/oauth/google.py`](https://github.com/KhulnaSoft/AutoAGI/blob/master/autoagi/backend/backend/integrations/oauth/google.py)

```python title="backend/integrations/oauth/google.py"
--8<-- "autoagi/backend/backend/integrations/oauth/google.py:GoogleOAuthHandlerExample"
```
...
--8<-- "autoagi/backend/backend/blocks/google/_auth.py:GoogleOAuthIsConfigured"
```
...
1. **Define `webhook_config`** in your block's `__init__` method.
    ...
    --8<-- "autoagi/backend/backend/blocks/github/triggers.py:example-webhook_config"
    ```
    </details>
    ...
    --8<-- "autoagi/backend/backend/data/block.py:BlockWebhookConfig"
    ```
    </details>
...
2. **Define event filter input** in your block's Input schema.
    ...
    ```python title="backend/blocks/github/triggers.py"
    --8<-- "autoagi/backend/backend/blocks/github/triggers.py:example-event-filter"
    ```
    </details>
...
4. **Include payload field** in your block's Input schema.
    ...
    ```python title="backend/blocks/github/triggers.py"
    --8<-- "autoagi/backend/backend/blocks/github/triggers.py:example-payload-field"
    ```
    </details>
...
--8<-- "autoagi/backend/backend/integrations/webhooks/_base.py:BaseWebhooksManager1"

--8<-- "autoagi/backend/backend/integrations/webhooks/_base.py:BaseWebhooksManager2"
--8<-- "autoagi/backend/backend/integrations/webhooks/_base.py:BaseWebhooksManager3"
--8<-- "autoagi/backend/backend/integrations/webhooks/_base.py:BaseWebhooksManager4"
--8<-- "autoagi/backend/backend/integrations/webhooks/_base.py:BaseWebhooksManager5"
```
...
--8<-- "autoagi/backend/backend/integrations/webhooks/__init__.py:WEBHOOK_MANAGERS_BY_NAME"
```
...
<details>
<summary>
GitHub Webhook triggers: <a href="https://github.com/KhulnaSoft/AutoAGI/blob/master/autoagi/backend/backend/blocks/github/triggers.py"><code>blocks/github/triggers.py</code></a>
</summary>

```python title="backend/blocks/github/triggers.py"
--8<-- "autoagi/backend/backend/blocks/github/triggers.py:GithubTriggerExample"
```
</details>

<details>
<summary>
GitHub Webhooks Manager: <a href="https://github.com/KhulnaSoft/AutoAGI/blob/master/autoagi/backend/backend/integrations/webhooks/github.py"><code>integrations/webhooks/github.py</code></a>
</summary>

```python title="backend/integrations/webhooks/github.py"
--8<-- "autoagi/backend/backend/integrations/webhooks/github.py:GithubWebhooksManager"
```
</details>
...
3. **Protected IP Ranges**:
   The wrapper denies requests to these networks:
    ...
    --8<-- "autoagi/backend/backend/util/request.py:BLOCKED_IP_NETWORKS"
    ```
...
By following these steps, you can create new blocks that extend the functionality of the AutoAGI Agent Server.

## Blocks we want to see

Below is a list of blocks that we would like to see implemented in the AutoAGI Agent Server. If you're interested in contributing, feel free to pick one of these blocks or chose your own.
...
- Google sheets - [~~Read/Append~~](https://github.com/KhulnaSoft/AutoAGI/pull/8236)
- Email - Read/Send with [~~Gmail~~](https://github.com/KhulnaSoft/AutoAGI/pull/8236), Outlook, Yahoo, Proton, etc
...
- External Agents - Call other agents similar to AutoAGI
