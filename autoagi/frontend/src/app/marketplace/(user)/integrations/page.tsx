...
import { CredentialsProviderName } from "@/lib/autoagi-server-api";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
    ...
    async (
      ...
      if (result.deleted) {
        if (result.revoked) {
          ...
          toast({
            title: "Credentials deleted from AutoAGI",
            description: `You may also manually remove the connection to AutoAGI at ${provider}!`,
            duration: 3000,
          });
        }
        setConfirmationDialogState({ open: false });
